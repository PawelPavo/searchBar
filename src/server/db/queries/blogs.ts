...
const tagSearch = (tagid: number) => Query<BlogsT[]>('SELECT blogs.*, users.email as name, tags.name as tag_name, blogTags.tagid FROM blogs JOIN users ON users.id = blogs.userid LEFT JOIN blogTags ON blogTags.blogid = blogs.id LEFT JOIN tags ON tags.id = blogTags.tagid WHERE tagid = ?', [tagid])
...
const find = (content:any) => Query<BlogsT[]>(`SELECT blogs.*, users.email as name, tags.name as tag_name, blogTags.tagid FROM blogs JOIN users ON users.id = blogs.userid LEFT JOIN blogTags ON blogTags.blogid = blogs.id LEFT JOIN tags ON tags.id = blogTags.tagid WHERE content LIKE ?`,[`%${content}%`])
